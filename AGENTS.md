...
- **item metadata via TSDoc** (default export + JSDoc). the parser extracts `name`, `description`, `props` (from the function's destructured params + inline object type literal + per-prop JSDoc), and `examples` (from `@example` blocks with optional `# Title` heading and ` ```tsx ` code fences). parser gotchas: a literal `@example` inside example code breaks the splitter; `https://` URLs inside JSDoc get truncated by the comment stripper.
...
- `<tier>/<name>/index.tsx` — default-exported function with TSDoc (the modo item).
- `<tier>/<name>/<name>.tsx` — vendored upstream source (shadcn/FF pattern) imported by the adapter.
...
`data-modo="app"`, `"sidebar"`, `"sidebar-nav"`, `"sidebar-section"`, `"sidebar-section-title"`, `"sidebar-section-items"`, `"sidebar-item"`, `"content"`, `"panel"`, `"header"`, `"section"`, `"section-title"`, `"page-title"`, `"page-lead"`, `"swatch"`, `"tokens-header"`, `"tokens-count"`, `"token-row"`, `"token-row-value"`, `"color-grid"`, `"color-stack"`, `"color-tile"`, `"color-tile-name"`, `"color-tile-pair"`, `"color-tile-value"`, `"hero"`, `"hero-stats"`, `"bento"`, `"bento-card"`, `"bento-stage"`, `"bento-meta"`, `"bento-title"`, `"bento-count"`, `"bento-desc"`, `"bento-empty"`, `"token-preview"`, `"token-chip"`, `"example-card"`, `"example-card-meta"`, `"example-card-stage"`, `"example-toggle"`, `"example-code"`, `"prop-table"`, `"raw-json"`. the lib sets structural styles for all of these; the user styles their own components via the same attrs.

 ...
 * `bunx shadcn@latest add @fluid/input-message` (shadcn CLI 4.21.0) into a scratch
  ...
  (
    ...
    useEffect(() => {
      ...
      if (prev === "streaming" && status === "idle" && queueArr.length > 0) {
        ...
        onQueueChange?.(rest);
        // `next!` — length checked above (noUncheckedIndexedAccess skew).
    ...
    const moveQueued = useCallback(
      (item: QueuedMessage, dir: -1 | 1) => {
        ...
        const next = [...cur];
        // Non-null — indices bounds-checked above (noUncheckedIndexedAccess skew).
      ...
      (e: ReactKeyboardEvent<HTMLTextAreaElement>) => {
        ...
        if (
          ...
          if (activeSuggestion != null) {
            ...
            if (e.key === "Enter") {
              e.preventDefault();
              // Non-null — activeSuggestion is a tracked index into
              // suggestionsArr (noUncheckedIndexedAccess skew).

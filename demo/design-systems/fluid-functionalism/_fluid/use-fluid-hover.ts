 ...
 * `bunx shadcn@latest add @fluid/use-fluid-hover` (shadcn CLI 4.21.0) into a scratch
 * scaffold. Local modifications: `@/…` imports rewritten to relative paths for the modo layout; `framer-motion` imports rewritten to `motion/react`; `"use client"` directives dropped (non-RSC). Type-skew fixes for this workspace's pinned deps: sessionRef typed MutableRefObject<number> (registry authored against React 19 ref types; workspace pins @types/react 18).
 */

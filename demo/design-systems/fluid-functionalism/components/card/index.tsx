 ...
 * `bunx shadcn@latest add @fluid/card` (shadcn CLI 4.21.0) into a scratch
 * scaffold. Local modifications: `@/…` imports rewritten to relative paths for the modo layout; `framer-motion` imports rewritten to `motion/react`; `"use client"` directives dropped (non-RSC). `next/link` replaced with plain anchors (no Next.js in modo). modo item docs: TSDoc on Card, default export, compound statics (Card.Group, Card.Header, …).
 */
...
/**
 * Fluid Functionalism Card — the shadcn compositional card with the @fluid
 * layer on top: an Elevated surface with a stepped shadow ladder, a
